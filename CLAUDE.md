...
Run the CLI (prompts interactively for a directory path if omitted; see `main.py::parse_args` for the full flag set — `--target-dir`, `--dry-run`, `-y`, `--path-weight`, `--max-clusters`, `--keyword-ngram`, `--keyword-count`, `--undo`):
```bash
